    ...
    public int[][] addShip(Ship ship, boolean isHorizontal, int xStart, int yStart) {
        if (isHorizontal) { // if the ship is being placed horizontally on the x axis
            while (xStart + ship.getLength() > 9 || xStart < 0) { // if it is going to not fit in the xaxis
                System.out.println(
                        "This placement exceeds the x boundaries of the grid. Please enter a valid x-coordinate for the leftmost point of the "
                                + ship.getName() + " : ");
                xStart = input.nextInt();
            }
            while (yStart < 0 || yStart > 9) { // if it wont fit on the y axis
                System.out.println(
                        "This placement exceeds the y boundaries od the grid. Please enter a valid y-coordinate for this "
            ...
            while (yStart + ship.getLength() > 9 || yStart < 0) { // if it is going to not fit in the yaxis
                System.out.println(
                        "This placement exceeds the y boundaries of the grid. Please enter a valid y-coordinate for the topmost point of the "
                                + ship.getName() + " : ");
                yStart = input.nextInt();
            }
            while (xStart < 0 || xStart > 9) { // if it wont fit on the x axis
                System.out.println(
                        "This placement exceeds the x boundaries of the grid. Please enter a valid x-coordinate for this "
                                + ship.getName() + " to be placed along : ");
                xStart = input.nextInt();
            }
